...
def validate_handler(remote, args, work, halt, mem2=False):
    ...
    source.write_text("\n".join(statements)+"\n")
    subprocess.run([str(Path(args.assembler).resolve()), "--no-config", "--bug-fixes=true", "--set=extensions.branch_expressions=true", "--set=extensions.non_console_instructions=false", "--set=extensions.expression_syntax=true", "--set=extensions.implicit_sections=true", "--set=bug_fixes.additional_console_instructions=true", "--set=validation.reject_duplicate_labels=true", "--set=validation.strict_macro_calls=true", "--set=validation.reject_undefined_macros=true", "--set=validation.reject_address_annotations=true", "--set=validation.reject_data_overflow=true", "-i", "-q", str(source)], check=True, capture_output=True, timeout=15)
