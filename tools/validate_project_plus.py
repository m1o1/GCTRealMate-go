    ...
    for implementation, executable in [("go", args.assembler), ("cpp", args.reference)]:
        ...
        for entry, base in entries:
            ...
            if implementation == "go":
                flags += ["--no-config", "--bug-fixes="+args.bug_fixes, "--set=extensions.dot_op="+args.bug_fixes, "--set=extensions.branch_expressions="+args.bug_fixes, "--set=extensions.non_console_instructions=false"] + ["--set="+key+"="+args.bug_fixes for key in ['extensions.expression_syntax', 'extensions.implicit_sections', 'bug_fixes.additional_console_instructions', 'validation.reject_duplicate_labels', 'validation.strict_macro_calls', 'validation.reject_undefined_macros', 'validation.reject_address_annotations', 'validation.reject_data_overflow']]
            if base:
                flags += ["-a", "-b:"+base]
