...
func encodeDirectivePolicy(n node, bugFixes bool) ([]uint32, *fixup, error) {
	if bugFixes {
		return encodeDirective(n)
	}
	s := strings.ToUpper(strings.TrimPrefix(compact(n.text), "."))
	if len(s) == 3 && strings.HasPrefix(s, "GR") && strings.ContainsRune("0123456789ABCDEF", rune(s[2])) {
		return []uint32{0, 0}, nil, nil
	}
	if s == "ELSE" {
		return nil, nil, fmt.Errorf("legacy GCTRealMate cannot compile .ELSE; enable bug_fixes")
	}
	if s == "ELSE_RESET" {
		return nil, nil, nil
	}
	if strings.HasPrefix(s, "BA") || strings.HasPrefix(s, "PO") {
		rhs := s[2:]
		...
		if strings.HasPrefix(rhs, "=") && (strings.Contains(rhs, "PO+") || strings.Contains(rhs, "BA+")) ||
			...
			return nil, nil, fmt.Errorf("legacy GCTRealMate cannot parse this BA/PO qualifier; enable bug_fixes")
		}
	}
	...
	if len(words) == 2 && fix == nil && (strings.HasPrefix(s, "BA") || strings.HasPrefix(s, "PO") || strings.HasPrefix(s, "GR")) {
		...
		if kind == 0x82000000 || kind == 0x84000000 {
			words[0] &^= 15
		}
	}
